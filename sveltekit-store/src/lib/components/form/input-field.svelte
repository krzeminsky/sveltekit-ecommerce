...
<div class="relative w-full group flex gap-2 min-w-0 mt-6">
    ...
    <input {id} {type} {placeholder} {autocomplete} {required} class="flex-1 basis-0 min-w-0 h-full border-2 px-5 py-2 rounded-xl outline-none focus:border-gray-500 transition-all" bind:this={field} on:input={e => value = e.currentTarget.value} on:keydown />
    <slot />
</div>